...
Usually the MTBF is express in hours, which is not the current case, the distribution used is often the **weinbull**, that we use below.  
...
For information the bearings failures, which are exceptional as most bearings live longer that the device they are associated with. In case of bearing it means we deal with the MTTF *(mean time to failure)* rather than the *MTBF*. 
 ...
 We do two calculations for two functional locations concerning the distribution MTBF, the distribution in this case are mulimodal, the MTBF is taken as the lowest model of the distribution. 
...
The **weibull** distribution is usually used in reliability, the method is different than the expectation for MTBF. We build a model with both materials in this case. 
