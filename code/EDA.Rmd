...
The previous formula is a simple definition, other mentioned MTBF as the arithmetic value of reliability function R(t), which can be expressed as the *expected value of the density function f(t)* or $$ MTBF \space  = \space \int_{0}^{\infty} tf(t)dt \\ Density \space f(t) = \lambda e^{-\lambda t} $$

Usually the MTBF is express in hours, which is not the current case, the distribution used is often the **weinbull**, that has to be used with **MLE**, as done below. 

To build this document, the **SKF** document **Bearing damage and failure analysis** has been  used. 

For information the bearings failures, which are exceptional as most bearings live longer that the device they are associated with. In case of bearing it means we deal with the MTTF *(mean time to failure)* rather than the *MTBF*. 
 ...
 Before to go further the definition of the MTBF is worst to mentioned, that we use the number of failures to repair in a period of time, which is an arithmetic mean, we compare with the density and raise some question as we have a lower value using the density.  
 
 We do two calculations for two functional locations concerning the distribution MTBF, the distribution in this case are multimodal, the MTBF is taken as the lowest model of the distribution. 
 
**One point**: It has been difficult to extract the distribution of failure in bearings, and some censuring have been applied. 
...
The distribution of the failures for the  floc **3021BRC501C503ELEM.PULL.TAPU** are as follow, we have a bimodal distribution here. One should be careful as the number of sample is low, in case of *lagging* we have only few samples, therefor some enhancement such as *bootstrap* must be used. 

We can notice a differences between the bearing  failure and lagging failure, what is surprising is the failures happening around thousand days. It seems that the bearings or the belts have changed. 
...
The non zero distribution is as follow, still the maximum of the failures happen in less than 200 days. 
...
The following visual shows the distribution of time between failure, we can notice the first modes are similar. The material **07187** has a long time between failure at the extreme, using the calculated MTBF will therefore have a values lot higher that the first mode. 
...
For both materials the time to failure is as follow, with no right censure. 
...
In case of the first material, we could censored the extremes, and then do a calculation with Weinbull, which will give us the reliability that we can transform in **MTBF**. 

The **weinbull** distribution is usually used in reliability, the method is different than the expectation for MTBF. We build a model with both materials in this case. 
...
In the previous paragraph we use one functional location, in this section we use **3074REC1PCONVELEM .BELT**, in this function location we have more than 50 failures. In this case we deal only with pulley.

```{r functional, echo=FALSE, warning =FALSE}
...
The first pulleys description is as below, we can see that we deal with multiple pulleys *(Four)*, which make the analysis interesting and the time between failure could be long, taking all the pulley we have a bimodal distribution.  
...
```{r dsp_pulley_graph, echo=FALSE, message=FALSE, warning= FALSE}
...
# we calculate the mode, the dataset is alreaday group by material_description 
...
If we use the MTBF formula which is challenging as we have period longer than a year. we have 2,75 failures per year. To build the distribution we have to consider the bimodal distribution, knowing that we have lower probability at 350 days. 
