...
In this document, we explore the **MTBF** based on a data set from **Rio**. The main aim was about how to calculate the **MTBF**, which could be valid. To build this document, the **SKF** document **Bearing damage and failure analysis** has been  used. 
 ...
 Before to go further the definition of the MTBF is worst to mentioned, it is the number of failures to repair in a period of time, it is an arithmetic mean, so nothing to do with distribution as mentioned in this document.  Its meaning is therefore restricted. 
...
We show the bearing failure with different view, this distribution is close to poisson, which could be approximated. The main question is how can we build a point estimate that is MTBF out of such a sample?
