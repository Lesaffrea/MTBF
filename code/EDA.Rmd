...
In the next lines we do bootstrap on the number of days, we should not have a lot of difference with the orignal data as we have ```r nrow(one_material)``` samples. We calculate the confidence interval on the boostrap as we have a normal distribution in this case. The CI is pretty small. The following visual is a bootstrap of the subset and therefore give the standard deviation for the population. In this case the mean will be of 134 instead of 133 for the subset.  
...
In th previous paragraph we use one functional location, in this section we use **3074REC1PCONVELEM .BELT**, in this function location we have more than 50 failures. I=n this case we deal only with pulley.
...
If we take the mode of distribution as the inspection time we have the following values, we can notice that the MTBF is higher than the mode MTBF, with 40 days diffrences, this could be an issue if we want to avoid the failure. 

| Material  |  Distribution MTBF |  MTBF |
|-----------|--------------------|-------|
| ```r mode_functional_3074$$material_description[1]``` | ```r mode_functional_3074$mode[[1]][1]``` | ```r `mode_functional_3074$mtbf[1] ```|
| ```r mode_functional_3074$$material_description[2]``` | ```r mode_functional_3074$mode[[2]][1]``` | ```r `mode_functional_3074$mtbf[2] ```|
| ```r mode_functional_3074$$material_description[3]``` | ```r mode_functional_3074$mode[[3]][1]``` | ```r `mode_functional_3074$mtbf[3] ```|
| ```r mode_functional_3074$$material_description[4]``` | ```r mode_functional_3074$mode[[4]][1]``` | ```r `mode_functional_3074$mtbf[4] ```|
