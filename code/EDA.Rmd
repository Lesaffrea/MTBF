...
In the next lines we do bootstrap on the number of days, which will give us the confidence interval, which is this case is pretty wide but normally distributed as expected. The following visual is a bootstrap of the subset and therefore give the standard deviation for the population, in this case the mean will be of 127 instead of 133 for the subset.  
...
If we do calculate the original MTBF we have a value of ```r ((one_material$scheduled_start[37] - one_material$scheduled_start[1])/ nrow(one_material))[1]```. We have a difference if we take the distribution rather than the arithmetic mean. 
