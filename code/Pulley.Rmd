...
This document analyses the pulley failure, by failure we mean **MTBF**, using distribution instead of the geometric mean. In this document we shall try pulley with **Weinbull** and two parameters, The issue we face is the low amount of failures for some **floc**, we then check the technical specificatio of the device. 
...
The distribution for those devices is as follow, we still have a standard deviation of eight, which mean the lower extrem is as 12 days, which make inspection quite often. If the failure was recurring if will be perhaps better to change. 
