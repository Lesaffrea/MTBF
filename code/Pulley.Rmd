...
In this sample rebuilt with selection of material description, we have same devices with significant difference in time to failure. It means we have conditions, which will reduce the time to failure in this case. It appears that we have few exceptions with low time to failures, which must be analysed.
...
This device is across multiple functional locations but same plant, the functional locations are as follow, we can notice the difference in time to failure inside a functional location, which add a complication. 
...
We make the hypothesis that the environment influences the time between failure, therefore we have a factor to define to do so we consider the full data set with bins. One challenge is the number of functional location of more than 1500. But the model is able to deal with this number. 
...
We have a prediction on numeric and therefore we recalibrate, the confusion matrix is shown below (* we certainly over-fit in this case *), but we show that we can have a pretty good prediction on class of time to failure. 
...
The previous chapter does not use a test set, in this section we shall use a training set made of .9 of the initial data set and a test set. We do not manage without rebuilding models as shown below, we use a tree methods and therefore the model could change drastically with few data changes as it is the case here.

```{r final_model, echo=FALSE,warning=FALSE, message=FALSE}
set.seed(998)
material_data <- material_data_set |>
                     dplyr::mutate( pulley_category = as.integer(factor(pulley_category)),
                                  functional_location = as.integer(factor(functional_location))) |>
                     dplyr::select(pulley_category, functional_location , bin_time)
        
        
        
inTraining <- createDataPartition(material_data$bin_time, p = .9, list = FALSE)
training <- material_data[ inTraining,]
testing  <- material_data[-inTraining,]
...
In this section, we build an ensemble model, we use **caretEnsemble** to build the ensemble. We can notice that the prediction with three models **glm**, **random forest** and **boosting tree*  is not accurate. We shall never go toward one or four. 

```{r ensemble_caret, echo=FALSE, warning=FALSE, message=FALSE}
# we have factor not in test
set.seed(998)
material_ensemble <- material_data  |> 
                                  dplyr::mutate( pulley_category = factor(pulley_category),
                                                functional_location = factor(functional_location)) |>
                                  dplyr::select(pulley_category, functional_location, bin_time ) |>
                                  dplyr::filter( !is.na(functional_location)) |>
                                  dplyr::mutate( bin_num = as.integer( bin_time)) |>
                                  dplyr::select(-bin_time)

inTraining <- createDataPartition(material_ensemble$bin_num, p = .9, list = FALSE)
ensemble_training <- material_ensemble[ inTraining,]
ensemble_testing  <- material_ensemble[-inTraining,]

my_control <- trainControl(
  method="boot",
  number=25,
  savePredictions="final",
  index=createResample(ensemble_training$bin_num, 25)
  )

# gradient boostin and rf 
model_ensemble <- caretList(
                  bin_num~., 
                  data= ensemble_training,
                  trControl =my_control,
                  methodList=c("glm", "rpart","xgbTree")
                  )

predict_ensemble <- as.data.frame(predict(model_ensemble, newdata=ensemble_testing , verbose=FALSE))
...
predict_ensemble_2 <- as.data.frame(predict(stack_ensemble, newdata=ensemble_testing, verbose=FALSE)) 

dsp_predict_org <-data.frame( predict = predict_ensemble_2[,1],
                             org =ensemble_testing$bin_num,
                             index= seq(1,nrow(predict_ensemble_2),1))

ggplot( data = dsp_predict_org) +
        geom_point( aes(x=index,y= predict), col="orange") +
        geom_point(aes(x=index, y=org), col="blue") +
        theme_minimal()+
        labs(title="Prediction stacking")

dsp_predict_org$residual_test <- dsp_predict_org$predict -dsp_predict_org$org 
```

The residual versus the predicted is show below, we have an increase  in residual further in value we predict. 

```{r ensemble_residual, echo=FALSE}
ggplot(dsp_predict_org, aes(x = predict, y = residual_test)) + 
  geom_pointrange(aes(ymin = 0, ymax = residual_test), col="orange") + 
